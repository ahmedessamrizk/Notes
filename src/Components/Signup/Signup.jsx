...
export default function Signup() {
    ...
    function checkInputs(newUser , e)
    {
        const schema = Joi.object({
            ...
            email: Joi.string().required().email({ minDomainSegments: 2, tlds: { allow: ['com', 'net'] } }).required(),
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-6 position-relative">
                                <div className="floating-label-group">
                                    <input autoComplete="off" autoFocus required onChange={getUser} typeof="text" className='form-control' id='first_name'/>
                                    <label className="floating-label">First Name</label>
                                    <i className="fa-solid position-absolute"></i> 
                                    <p className='text-danger wrong-input mb-2' id='first_name'>{getError('first_name')} </p>
                                </div>
                            </div>
                            <div className="col-md-6 position-relative">
                                <div className="floating-label-group mt-3">
                                    <input autoComplete="off" autoFocus required onChange={getUser} typeof="text"  className='form-control' id='last_name'/> <label className="floating-label">Last Name</label> <i className="fa-solid position-absolute"></i> <p className='text-danger wrong-input mb-2' id='last_name'>{getError('last_name')}</p>
                                </div>
                            </div>
                            ...
                            <div className="col-md-12 position-relative">
                                <div className="floating-label-group mt-3">
                                    <input autoComplete="off" autoFocus required onChange={getUser} typeof="number"  className='form-control' id='age'/><label className="floating-label">Age</label> <i className="fa-solid position-absolute"></i> <p className='text-danger wrong-input mb-2' id='age'>{getError('age')}</p>
                                </div>
                            </div>
                            <div className="col-md-12 position-relative">
                                <div className="floating-label-group mt-3">
                                    <input autoComplete="off" autoFocus required onChange={getUser} typeof="password"  className='form-control' id='password'/> <label className="floating-label">Password</label> <i className="fa-solid position-absolute"></i> <label className="floating-label">Password</label> <p className='text-danger wrong-input mb-2' id='password'>{getError('password')}</p>
                                </div>
                            
                            </div>
                            
                            <div className="position-relative col-md-12">
                            {apiFlag?<button typeof='submit' className='btn w-100 btn-info my-3 saveData btnMain'> Waiting... </button> : <button typeof='submit' className='btn w-100 btn-info my-2 saveData btnMain'> Sign Up </button>}
